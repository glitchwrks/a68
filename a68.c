...
3.5! Aug 2019   added col-1 '*' comment support HRJ
     Jan 2022	HRJ NOT result evaluated incorrectly, test ff80 -> ff00
	 Feb 2022	HRJ now flags no-operands as error S
...
char **argv;
{
    ...
    printf("fixes for LCC/Windows and improvement by Herb Johnson Feb 5 2022\n");
...
static void normal_op(void)
{
	...
	switch (lex() -> attr & TYPE) {
	    ...
	    case IMM:	bytes = 3;
			...
			if (attrib & IMM8) {
			    --bytes;
			    if (operand > 0xff && operand < 0xff80) { /* to trap -128...-255 */
				error('V'); /* was followed by a return */
			    }
			}
			attrib &= REGREQ;  break;
	    ...
	    case VAL:	unlex();
			...
			if (attrib & REL) {
			    ...
			    if (operand > 0x7f && operand < 0xff80) { /* ff80 is OK ?*/
				error('B');  return;
			    }
			    attrib = NONE;  break;
			}
	    ...
	    case EOL:
			...
			if ((bytes==1) && (attrib != NONE)) { /* need operand but have none zyzzy */
				obj[0] = opcode; obj[1]=0; obj[2]=0;
				error('S'); /* keep going */
			}
